...
export function GerenciarTiposPostes() {
  ...
  const handleSavePostType = async (postTypeData: { name: string; code?: string; description?: string; shape?: string; height_m?: number; price: number }) => {
    setOperationLoading(true);
    try {
      if (editingPostType) {
        await updatePostType(editingPostType.id, postTypeData);
        showMessage('success', 'Tipo de poste atualizado com sucesso!');
      } else {
      ...
      <div className="flex justify-between items-center flex-shrink-0">
        ...
        <div className="flex space-x-3">
          <button 
            ...
            disabled={loadingPostTypes || operationLoading}
                    ...
                    <tr key={postType.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        <button
                          ...
                          disabled={operationLoading || deletingId === postType.id}
